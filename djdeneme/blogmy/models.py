...
class myblogger(models.Model):
    ...
    def get_favorite_usernames(self):  # favorilerdeki usernameleri çeker ve javascript için bir liste yapar
        gecici = []
        temp = ""
        for i in range(len(self.get_added_favorite_user())):
            if i != (len(self.get_added_favorite_user())- 1):
                gecici.append(self.get_added_favorite_user()[i]+ "~")
            else:
                gecici.append(self.get_added_favorite_user()[i])
